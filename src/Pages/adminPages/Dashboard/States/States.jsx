...
async function CreateState(logo) {
    const name = document.querySelector("#name").value;
    if (!logo) {
        return document.querySelector("#Formerror").textContent = "Please Insert An Image Of The State"
    }
...
function EditCom(com) {
    window.location.href = "/Admin/State/edit/" + com
...
export default function States() {
    ...
    async function DeleteCommunities() {
        const response = await fetch("/api/delete-state", {
            method: "POST", 
            headers: {
                        ...
                        <tbody>
                            {visibleCommunities.map((dev) => (
                                <tr key={dev.id}>
                                    ...
                                    <td>{dev.id}</td>
                                    <td><img src={`/api/file/${dev.image}`} style={{maxHeight: "100px"}} /></td>
                                    <td>{dev.name}</td>
                                </tr>
                            ))}
                        </tbody>
                ...
                createForm && (
                    <div style={{position: "absolute", top: "0px", left: "0px", bottom: "0px", right: "0px", background: "rgb(0, 0, 0, 0.7)", display: "flex", alignItems: "center", justifyContent: "center"}} className="newPropCont">
                        ...
                        <div style={{width: "70%", height: "90%", background: "white", borderRadius: "10px", overflowY: "auto", paddingBottom: "30px", zIndex: "7"}}>
                            <h1 style={{textAlign: "center", color: "#333"}}>Create New State</h1>
                            ...
                            <div style={{display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", marginTop: "50px", gap: "10px"}} onFocus={(e) => {e.target.style.borderColor = "#004274"}} onBlur={(e) => { e.target.style.borderColor = "#ccc" }}>
                                <input type="text" style={{border: "1px solid #ccc", background: "#eee", borderRadius: "7px", padding: "10px 20px", color: "#333", fontWeight: "600", outline: "none", transition: "all .5s", width: "80%"}} placeholder="State's Name" id="name" />

                                
                                <button style={{width: "84%", outline: "none", border: "none", borderRadius: "7px", padding: "10px 20px", background: "#04418b", color: "#fff", cursor: "pointer", fontSize: "1.1rem", fontWeight: "550"}} onClick={() => {CreateState(uploadedLogo)}}>Create</button>
                            </div>
                        </div>
                    </div>
                )
